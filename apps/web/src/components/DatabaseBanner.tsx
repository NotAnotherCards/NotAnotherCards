...
export function DatabaseBanner() {
  ...
  return (
    <div className="w-full border-b transition-all duration-300">
      {status === "loading" && (
        <div className="bg-blue-500/10 border-blue-500/20 dark:text-blue-300 px-4 py-2.5 flex items-center gap-2.5 text-xs animate-in fade-in duration-200">
      ...
      {status === "taken-over" && (
        <div className="bg-amber-500/10 border-amber-500/20 dark:text-amber-300 px-4 py-3 flex items-center justify-between gap-3 text-sm">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 rounded text-white font-medium hover:bg-amber-600 active:bg-amber-700 transition-colors text-xs cursor-pointer shadow-sm"
          >
      ...
      {status === "error" && isOpfsBlocked(error) && (
        <div className="bg-amber-500/10 border-amber-500/20 dark:text-amber-300 px-4 py-3 flex items-center gap-2 text-sm">
          <AlertCircle className="size-4 text-amber-500 shrink-0" />
          <span>
      ...
      {status === "error" && !isOpfsBlocked(error) && (
        <div className="bg-destructive/10 border-destructive/20 dark:text-red-400 px-4 py-3 flex items-center justify-between gap-3 text-sm">
          <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleReconnect}
            className="flex items-center gap-1.5 px-3 py-1 rounded text-destructive-foreground font-medium hover:bg-destructive/90 active:bg-destructive transition-colors text-xs cursor-pointer shadow-sm"
          >
            <RefreshCw className="size-3" />
            Retry
          </button>
