...
if (!model || model.startsWith('--')) {
  console.error(
    'usage: node model-test.mts <model> [--nothink] [--base <url>] [--key <key>]',
  );
  process.exit(1);
}
...
const PROMPTS: Record<string, string> = {
  spanish:
    ...
    'for the topic "Spanish preterite tense for beginners". ' +
    FORMAT,
  biology:
    'You generate flashcards for a spaced-repetition app. Create 5 flashcards ' +
    'for the topic "photosynthesis, for a high school biology student". ' +
    FORMAT,
  german:
  ...
  history:
    'You generate flashcards for a spaced-repetition app. Create 5 flashcards ' +
    'for the topic "key events of the French Revolution with dates". ' +
    FORMAT,
  programming:
    'You generate flashcards for a spaced-repetition app. Create 5 flashcards ' +
  ...
  music:
    'You generate flashcards for a spaced-repetition app. Create 5 flashcards ' +
    'for the topic "music intervals for beginners (thirds, fifths, octaves)". ' +
    FORMAT,
};

async function ask(
  prompt: string,
): Promise<{ content: string; tokens: number; seconds: number }> {
  const body = {
    model,
...
if (topic && !(topic in PROMPTS)) {
  console.error(
    `unknown topic "${topic}"; available: ${Object.keys(PROMPTS).join(', ')}`,
  );
  process.exit(1);
}
...
for (const [name, prompt] of Object.entries(selected)) {
  ...
  console.log(
    JSON.stringify(cards ?? content.slice(0, 600), null, 1).slice(0, 1600),
    '\n',
  );
}
